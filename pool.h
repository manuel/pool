/* Simple memory pool for event processing and other applications: all
   memory needed for handling a request (or doing a particular
   operation) is allocated from the pool.  When the request is served
   the pool is reset to a standard size.  This means that most
   requests can be served without a call to malloc.
